(() => {
  ...
  const BASEMAPS = [
    { id: "osm", name: "OpenStreetMap", url: "https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", attr: "(c) OpenStreetMap contributors", maxZoom: 19 },
    { id: "carto-light", name: "Carto Light", url: "https://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}{r}.png", attr: "(c) OpenStreetMap (c) CARTO", maxZoom: 20 },
    { id: "carto-dark", name: "Carto Dark", url: "https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png", attr: "(c) OpenStreetMap (c) CARTO", maxZoom: 20 },
    { id: "topo", name: "OpenTopoMap", url: "https://{s}.tile.opentopomap.org/{z}/{x}/{y}.png", attr: "(c) OpenStreetMap (c) OpenTopoMap (CC-BY-SA)", maxZoom: 17 }
  ];
  ...
  const LAYERS = [
    ...
    { id: "bridge-damage", name: "Bridge damage assessment", meta: "HOT bridge survey", url: "https://production-raw-data-api.s3.amazonaws.com/ISO3/NPL/combined/hot_flood_npl_bridge_damage.geojson", color: "#b45309", fillOpacity: 0.5, weight: 2, defaultOn: true, heavy: false },
    ...
    { id: "destroyed-features", name: "Destroyed and damaged features", meta: "OSM destroyed/damaged - large", url: "https://production-raw-data-api.s3.amazonaws.com/ISO3/NPL/destroyed_features/hot_flood_npl_destroyed_features_osm.geojson", color: "#7f1d1d", fillOpacity: 0.4, weight: 1, defaultOn: false, heavy: true }
  ];
  ...
  if (!window.L) {
    setStatus("Leaflet failed to load. Refresh the page.", "error");
    return;
  }
  ...
  BASEMAPS.forEach(function (b, i) {
    ...
    if (i === 0) {
      layer.addTo(map);
      if (creditEl) creditEl.textContent = b.attr + " - HOT/HDX damage data";
    }
  });
  ...
  function popupHtml(props) {
    ...
    const ordered = prefer.filter(function (k) { return keys.indexOf(k) !== -1; })
      .concat(keys.filter(function (k) { return prefer.indexOf(k) === -1; }).slice(0, 12));
    return "<table>" + ordered.map(function (k) {
      return "<tr><td>" + escapeHtml(k) + "</td><td>" + escapeHtml(String(props[k])) + "</td></tr>";
    }).join("") + "</table>";
  }
  ...
  function styleFeature(def, feature) {
    ...
    if (def.styleBy === "damage" && p.damage) {
      const key = String(p.damage).toLowerCase();
      color = DAMAGE_COLORS[key] || def.color;
    } else if (p.status) {
      const st = String(p.status).toLowerCase();
      if (st.indexOf("destroy") !== -1) color = "#7f1d1d";
      else if (st.indexOf("damage") !== -1) color = "#ea580c";
    }
    return { color: color, weight: def.weight, opacity: 0.9, fillColor: color, fillOpacity: def.fillOpacity };
  }
  ...
  function pointToLayer(def, feature, latlng) {
    ...
    return window.L.circleMarker(latlng, {
      radius: 5, color: color, weight: 1, fillColor: color, fillOpacity: 0.75
    });
  }
  ...
  async function loadLayer(def) {
    ...
    try {
      ...
      const leafletLayer = window.L.geoJSON(geojson, {
        style: function (f) { return styleFeature(def, f); },
        pointToLayer: function (f, ll) { return pointToLayer(def, f, ll); },
      ...
      st.loading = false;
      setStatus("Could not load " + def.name + ": " + err.message, "error");
      return null;
    }
  }
  ...
  function fitVisible() {
    ...
    Object.keys(layerState).forEach(function (id) {
      ...
      if (st.leafletLayer && map.hasLayer(st.leafletLayer)) group.push(st.leafletLayer);
    });
    if (!group.length) {
    ...
    try {
      map.fitBounds(window.L.featureGroup(group).getBounds(), { padding: [28, 28], maxZoom: 13 });
    } catch (e) {}
  }

  LAYERS.forEach(function (def) {
    layerState[def.id] = { def: def, leafletLayer: null, loaded: false, loading: false };
    ...
    label.innerHTML =
      '<span class="lname"><span class="swatch" style="background:' +
      def.color +
      '"></span>' +
      escapeHtml(def.name) +
      '</span><span class="lmeta">' +
      escapeHtml(def.meta) +
      "</span>";
    input.addEventListener("change", function () {
    ...
    if (def.defaultOn) {
      setTimeout(function () {
        setLayerVisible(def, true);
      }, 80);
    }
  });

  const btnFit = byId("btn-fit");
  if (btnFit) btnFit.addEventListener("click", fitVisible);
  ...
  if (btnPng) {
    btnPng.addEventListener("click", async function () {
      ...
      try {
        ...
        a.download = "rasuwa-flood-map.png";
  ...
  setTimeout(function () {
    ...
    Object.keys(layerState).forEach(function (id) {
      ...
      if (st.leafletLayer && map.hasLayer(st.leafletLayer)) any = true;
    });
    if (any) fitVisible();
  }, 2500);
  ...
  const FALLBACK_NEWS = [
    ...
    { title: "fAIr building damage assessment - Upper Trishuli and Bhotekoshi", url: "https://data.humdata.org/dataset/hot_flood_npl_buildings_damage", date: "HOT", source: "Humanitarian OpenStreetMap Team" },
    { title: "ReliefWeb - Rasuwa flood reports and maps", url: "https://reliefweb.int/", date: "ReliefWeb", source: "OCHA" }
  ];
  ...
  function renderNews(items, sourceLabel) {
    ...
    newsRail.innerHTML = "";
    if (!items.length) {
      newsRail.innerHTML = '<p class="news-empty">No headlines loaded. See official sources in the FAQ.</p>';
      if (newsMeta) newsMeta.textContent = "Unavailable";
      return;
    }
    ...
    if (newsMeta) newsMeta.textContent = sourceLabel || "Updated";
  }
  ...
  async function loadNews() {
    try {
      ...
      if (res.ok) {
        ...
        const arts = (data.articles || []).filter(function (a) {
          return a.url && a.title;
        });
        if (arts.length) {
          const items = arts.slice(0, 10).map(function (a) {
            var dateStr = "";
            if (a.seendate && a.seendate.length >= 8) {
              dateStr =
